...
namespace Quasar.Helpers.Downloading
{
    public class ModManager : ObservableObject
    {
        ...
        /// </summary>
        /// <returns>Success state</returns>
        public async Task<bool> TakeAction(ILog QuasarLogger, ObservableCollection<QuasarModType> _mod_types, Game _game)
        {
                ...
                if (Downloaded)
                {
                    ...
                    if (Extracted)
                    {
                        ...
                        if (Processed)
                        {
                            ...
                            {
                                QuasarLogger.Debug("Could not get Screenshot");
                            }

                            await Scan(_mod_types, _game);
        ...
        public async Task<bool> Scan(ObservableCollection<QuasarModType> QuasarModTypes, Game Game)
        {
            ...
            await Task.Run(() =>
            {
                ScannedContents = FileScanner.ScanMod(ModFileManager.LibraryContentFolderPath, QuasarModTypes, Game, LibraryItem);
            });
            return true;
        }
        #endregion
