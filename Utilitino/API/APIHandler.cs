...
namespace Utilitaries.API
{
    ...
    public class APIRequest
    {
        ...
        /// <returns></returns>
        public static async Task<int> GetScreenshot(string _ItemType, string _ItemID, string Guid, string DestinationDirectory)
        {
            string downloadURL = "";
            ...
            try
            {
                using (HttpClient webClient = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        string imageSource = DestinationDirectory + @"\Library\Screenshots\" + Guid + "." + downloadextension;
                        string convertedImageSource = DestinationDirectory + @"\Library\Screenshots\" + Guid + ".webp" ;

                        await DownloadScreenshot(downloadURL, imageSource);
                        ...
                        if(downloadextension != "webp")
                        {
                            try
                            {
                                using (FileStream fsSource = new FileStream(imageSource, FileMode.Open, FileAccess.Read))
                                {
                                    ...
                                    using (var webPFileStream = new FileStream(convertedImageSource, FileMode.Create))
                                    {
                                ...
                                if (File.Exists(convertedImageSource))
        ...
        public static async Task<bool> DownloadScreenshot(string URL, string DestinationFilePath)
        {
            try
            {
                //File Download
